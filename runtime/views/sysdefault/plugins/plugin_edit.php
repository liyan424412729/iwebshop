		...
		<form action="<?php echo IUrl::creatUrl("/plugins/plugin_update");?>" method="post">
			...
			<table class="form_table" cellpadding="0" cellspacing="0">
				...
				<tr>
					<th></th>
					<td>
						<button class="submit" type='submit' onclick='$("tr:hidden").remove();'><span>确 定</span></button>
					</td>
				</tr>
			</table>
		</form>
	...
	<td>
		<%if(item['type'] == "radio"){%>
			...
			<%tempVal = valueItems[tempKey]%>
				<label class="attr"><input type="radio" name="<%=key%>" value="<%=tempVal%>" checked="checked" /><%=tempKey%></label>
			<%}%>
		...
		<%}else if(item['type'] == "select"){%>
			<select class="auto" name="<%=key%>" pattern="<%=item['pattern']%>" <%if(item['bind']){%>onchange="bindCallback(this,<%=JSON().stringify(item['bind'])%>);"<%}%>>
			<%for(var tempKey in valueItems){%>
			<%tempVal = valueItems[tempKey]%>
