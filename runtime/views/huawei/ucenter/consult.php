...
<?php $queryConsultList = Api::run('getUcenterConsult',$this->user['user_id'])?>
<?php $queryConsult = $queryConsultList->find()?>
<?php if($queryConsult){?>
<section class="uc_table">
	<table>
		...
		<tbody>
			<?php foreach($items=$queryConsult as $key => $item){?>
			<tr>
				...
				<td><?php if($item['status']==1){?><span class="blue" onclick='show_msg(this, <?php echo JSON::encode($item);?> )'>查看</span><?php }else{?>未回复<?php }?></td>
			</tr>
			<?php }?>
