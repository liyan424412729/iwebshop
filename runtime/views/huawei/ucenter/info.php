...
<section class="user_form">
	<form action='<?php echo IUrl::creatUrl("/ucenter/info_edit_act");?>' method='post' name='user_info'>
		<dl>
			<dt>头像：</dt>
			<dd><?php $user_ico = $this->user['head_ico']?>
				<div class="user_ico">
		...
		<?php if($this->memberRow['group_id']){?>
		<dl>
			<dt>会员组：</dt>
			<dd>
				<?php $query = new IQuery("user_group");$query->where = "id = {$this->memberRow['group_id']}";$items = $query->find(); foreach($items as $key => $item){?><?php echo isset($item['group_name'])?$item['group_name']:"";?><?php }?>
			</dd>
		</dl>
		<?php }?>
		<dl>
