...
<section class="uc_table">
	<table>
		...
		<tbody>
			<?php foreach($items=$addressList as $key => $item){?>
			<tr>
				...
				<td class="uc_tab_operation">
					<a href='javascript:void(0)' onclick='editAddress(<?php echo isset($item['id'])?$item['id']:"";?>)'>
						<i class="fa fa-pencil" style="font-size: 16px;"></i>
					</a>|
					<a href="javascript:void(0)" onclick="delModel({link:'<?php echo IUrl::creatUrl("/ucenter/address_del/id/".$item['id']."");?>'});">
						<i class="fa fa-remove" style="font-size: 16px;"></i>
					</a>|
					<?php if($item['is_default']==1){?>
					<a class="striking" href="<?php echo IUrl::creatUrl("/ucenter/address_default/id/".$item['id']."/is_default/0");?>">取消默认</a>
					<?php }else{?>
					<a href="<?php echo IUrl::creatUrl("/ucenter/address_default/id/".$item['id']."/is_default/1");?>">设为默认</a>
					<?php }?>
				</td>
				<!-- 个人中心表格操作 -->
			</tr>
