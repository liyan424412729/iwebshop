...
<?php $querySuggestionList = Api::run('getUcenterSuggestion',$this->user['user_id'])?>
<?php $querySuggestion = $querySuggestionList->find()?>
<?php if($querySuggestion){?>
<section class="uc_table">
	<table>
		...
		<tbody>
			<?php foreach($items=$querySuggestion as $key => $item){?>
			<tr>
				...
				<td><a class="blue" onclick='show_msg(this,<?php echo JSON::encode($item);?>)'>查看</a></td>
			</tr>
			<?php }?>
			<tr id='show_msg' class="show" style="display:none">
						...
						<p id='content'></p>
						<div id="re_info">
							<strong><span id='admin'><?php echo $this->_siteConfig->name;?></span>在 [<span id='re_time'></span>] 回复:</strong>
							<p id='re_content'></p>
						</div>
					</div>
				</td>
