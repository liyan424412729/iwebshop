...
<?php $queryRefundmentDoc = $queryRefundmentDocList->find()?>
<?php if($queryRefundmentDoc){?>
<section class="uc_table">
	<table>
		...
		<tbody>
			<?php foreach($items=$queryRefundmentDoc as $key => $item){?>
			<tr>
				...
				<td class="uc_tab_operation">
					...
					<a href='javascript:delModel({link:"<?php echo IUrl::creatUrl("/ucenter/refunds_del/id/".$item['id']."");?>"})'>取消</a>
					|
					<?php }else{?>
					<a></a>&nbsp;&nbsp;
					<?php }?>
