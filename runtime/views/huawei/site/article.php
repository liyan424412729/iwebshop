...
<div class="bigweb">
	<section class="breadcrumb">
		<span>您当前的位置：</span> <a href="<?php echo IUrl::creatUrl("");?>">首页</a> > <?php if($catRow){?><?php echo isset($catRow['name'])?$catRow['name']:"";?><?php }else{?>最新资讯<?php }?>
	</section>
	<section class="web">
		<section class="article_main">
			<h2 class="main_head"><?php if($catRow){?><?php echo isset($catRow['name'])?$catRow['name']:"";?><?php }else{?>最新资讯<?php }?></h2>
			<div class="article_list">
				<ul>
					<?php foreach($items=$queryArticle->find() as $key => $item){?>
					<li>
						<a href="<?php echo IUrl::creatUrl("/site/article_detail/id/".$item['id']."");?>"><?php echo isset($item['title'])?$item['title']:"";?></a>
						<time><?php echo isset($item['create_time'])?$item['create_time']:"";?></time>
					</li>
					<?php }?>
				</ul>
