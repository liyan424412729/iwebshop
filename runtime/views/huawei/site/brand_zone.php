...
<?php $goodsObj = search_goods::find(' go.brand_id = '.$brandId,20);$resultData = $goodsObj->find();?>
<div class="bigweb">
	...
	<section class="web">
		<section class="main">
			<section class="about_brand">
				<h2 class="main_head">
					关于<mark><?php echo isset($brandRow['name'])?$brandRow['name']:"";?></mark>品牌
					<?php if($brandRow['url']){?><a class="barnd_web" href="<?php echo isset($brandRow['url'])?$brandRow['url']:"";?>"><i class="fa fa-home"></i></a><?php }?>
				</h2>
				<div class="barnd_info">
					<img class="brand_logo" src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$brandRow['logo']."/w/215/h/120");?>">
					<?php echo isset($brandRow['description'])?$brandRow['description']:"";?>
				</div>
			</section>
		...
		<aside class="bar">
			...
			<div class="bar_box">
				...
				<ul class="bar_goods ranking">
					<?php foreach($items=Api::run('getGoodsListBrandSum',array('#brandid#',$brandId),10) as $key => $item){?>
					<li>
			...
			<div class="bar_box">
				...
				<ul class="bar_goods">
					<?php foreach($items=Api::run('getCommendHotBrand',array('#brandid#',$brandId),10) as $key => $item){?>
					<li>
						<a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['id']."");?>">
							<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/60/h/60");?>" alt="<?php echo isset($item['name'])?$item['name']:"";?>">
							<strong><?php echo isset($item['name'])?$item['name']:"";?></strong>
			...
			<nav class="bar_box">
				...
				<ul class="bar_nav">
					<?php foreach($items=Api::run('getBrandList',10) as $key => $item){?>
